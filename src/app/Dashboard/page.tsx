...
import Notifier from '../component/reusable_component/Notification';
import { NotifierContext } from '../context/notificationContext';
import { getUserNotificationCount } from "../component/backend_component/TaskBackend";
...
interface DashboardPageProps {
   ...
   userTask: React.ReactNode;
   userDeletedFiles: React.ReactNode;
   weeklyData: React.ReactNode;
   helperViewTab: React.ReactNode;
   userFullTaskDetailsPreview: React.ReactNode;
   userTaskQueryPath: (dashboardBtn: string, dashboardRoute: string) => void
   showSearchFullResult: (userId: number, taskId: number, dashboardBtn: string) => void
}
...
export default function DashboardPage({
   ...
   const [results, setResults] = useState<[{ id: number, title: string }]>([{ id: 0, title: "" }]);
   ...
   function showNotificationStatus() {
      setShowNotification((prevCondition) => !prevCondition ? true : false)
   }
   ...
   const fullDataSearch = (id: number, userDashboardSearch: string) => {
      showSearchFullResult(userId as number, id, userDashboardSearch)
   }
   ...
   function ThemeChange() {
      ...
      if (userId !== null) {
         fetchNotificationCount(userId as number)
      }
      setTheme(newTheme);
      localStorage.setItem('theme', newTheme);
   }

   async function fetchNotificationCount(userId: number) {
      if (userId === null) return; // Make sure a valid userId is present
      try {
         const countResult = await getUserNotificationCount(userId);
         console.log("🚀 ~ fetchNotificationCount ~ countResult:", countResult);
         setNotificationCount(countResult); // Update state with the new count
      } catch (error) {
   ...
   const handleSearch = useCallback(async (query: string, userSearchBarDashboardBtn: string) => {
      ...
      try {
         ...
         const response = await fetch("/api/search", {
            ...
            body: JSON.stringify({ query, userId, userSearchBarDashboardBtn })
         });
         ...
         <section className={theme === 'light' ? "side-nav-container-whiteTheme" : "side-nav-container-blackTheme"}>
            <div className="side-nav">
               ...
               <div className="search_container">
                  ...
                  />
                  {(results[0].id !== 0 && results[0].title !== "" && searchDisplay) &&
                     <div className="search_result_container">
                        ...
                        <fieldset className="search_fieldset" >
                           ...
                           {(results[0].id !== 0 && results[0].title !== "") && results.map((item, index) => (
                              <div
